...
public class DAOKhachSan {
    ...
    public static ArrayList<KhachSan> getAll() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                tmp.setIdKhachSan(rs.getInt("Id"));
                tmp.setTenKhachSan(rs.getString("Ten"));
    ...
    public static boolean insert(KhachSan tmp) {
        try {
            ...
            stmt.setString(1, tmp.getTenKhachSan()); // Lấy giá trị ở getTen() gán vào vị trí ? số 1
            ...
            ResultSet rs = stmt.executeQuery(); // Thực thi câu lệnh insert bên trên
            if (rs.next()) {
                tmp.setIdKhachSan(rs.getInt("Id"));
            }
            con.close();
    ...
    public static boolean update(KhachSan tmp) {
        try {
            ...
            stmt.setString(1, tmp.getTenKhachSan());
            stmt.setString(2, tmp.getDiaChi());
            stmt.setString(3, tmp.getSoDienThoai());
            stmt.setInt(4, tmp.getCachTrungTam());
            stmt.setString(5, tmp.getMoTa());
            ...
            stmt.setInt(9, tmp.getIdThanhPho());
            stmt.setInt(10, tmp.getIdLoaiKhachSan());
            stmt.setInt(11, tmp.getIdKhachSan());
            stmt.executeUpdate();
            con.close();
        } catch (Exception e) {
            return false;
