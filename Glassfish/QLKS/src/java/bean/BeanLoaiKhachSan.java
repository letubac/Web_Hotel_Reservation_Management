...
public class BeanLoaiKhachSan implements Serializable {
    ...
    public static HashMap<Integer, String> hashLoaiKhachSan; // hashmap<key,value>

    @ManagedProperty(value = "#{beanKhachSan.listKhachSan}") // Ta có thể sử dụng giá trị của beanKS mà ko cần khởi tạo
    ...
    public BeanLoaiKhachSan() {
        ...
        for (LoaiKhachSan tmp : listLoaiKhachSan) {
            ...
            hashLoaiKhachSan.put(tmp.getId(), tmp.getTen()); // Thêm 1 cặp <key,value> vào hashmap
        }
    }
    ...
    public void insert(LoaiKhachSan tmp) throws IOException {
        ...
        if (dao.DAOLoaiKhachSan.insert(tmp)) {
            ...
            hashLoaiKhachSan.put(tmp.getId(), tmp.getTen()); //Khi thêm 1 loại ks thì cũng thêm vào hashmap
    ...
    public void update(LoaiKhachSan tmp) throws IOException {
        ...
        if (dao.DAOLoaiKhachSan.update(tmp)) {
            ...
            // Khi update thành công thì sẽ update luôn trong hash
